...
class TestRecommendation:
    ...
    def test__recommendation__given_valid_inputs__computes_totals_correctly(self, picnic_request, banana, chips):
        # ARRANGE / ACT
        # 2 bananas + 1 chips
    ...
    def test__recommendation__given_empty_quantities__raises_value_error(self, picnic_request):
        with pytest.raises(ValueError, match="quantities"):
            Recommendation(request=picnic_request, quantities={})

    def test__recommendation__given_quantity_less_than_one__raises_value_error(self, picnic_request, banana):
        with pytest.raises(ValueError, match="quantity"):
            Recommendation(request=picnic_request, quantities={banana: 0})

    def test__recommendation__given_product_not_in_request__raises_value_error(self, picnic_request):
    ...
    def test__recommendation__given_total_cost_exceeds_budget__raises_value_error(self, banana, chips):
    ...
    def test__recommendation__given_total_weight_exceeds_limit__raises_value_error(self, banana, chips):
